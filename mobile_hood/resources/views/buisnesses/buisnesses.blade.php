            ...
            <div class="card">
                <div class="card-header">
                    <div class="row align-items-center">
                        <div class="col col-md-6">{{ __('messages.buisnesses') }}</div>
                        <div class="col col-md-6">
                            <a href="{{ route('buisnesses.create') }}"
                            ...
                            <tbody>
                                @foreach ($buisnesses as $b)
                                    <tr>
                                        ...
                                        <td>
                                            <form method="post"
                                                action="{{ route('buisnesses.destroy', ['buisness' => $b->id]) }}">
                                                @csrf
                                                @method('DELETE')
                                                <a href="{{ route('buisnesses.show', ['buisness' => $b->id]) }}"
                                                    class="btn btn-primary btn-sm">{{ __('messages.show') }}</a>
                                                <a href="{{ route('buisnesses.edit', ['buisness' => $b->id]) }}"
