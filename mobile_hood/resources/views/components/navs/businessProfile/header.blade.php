<nav class="navbar navbar-expand py-0 border-bottom">
    <div class="container-fluid">
        <div class="d-flex align-items-center">
            <a href="{{ route('home') }}"><i class="bx bx-chevron-left fs-1 fw-medium opacity-75 text-dark pt-1"></i></a>
                ...
                <div class="modal-dialog modal-dialog-scrollable">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="fw-semibold" id="infoModalLabel">Información</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal"
                                aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <span id="lat" class="d-none">{{ $businessModel->location->lat }}</span>
                            <span id="lng" class="d-none">{{ $businessModel->location->lng }}</span>
                            <div class="map-container mb-3">
                                <div id="map"></div>
                            </div>
                            <div class="d-flex mb-4">
                                <div class="modal-logo-container border border-dark rounded-2 me-2">
                                    <img src="{{ $businessModel->getImageUrl() }}" alt="{{ $businessModel->name }} logo"
                            ...
                            @php
                                $openDays = $businessModel->hours->pluck('day_of_week')->toArray();
                            @endphp
                            @foreach ($days as $key => $day)
                                @php
                                    $isOpen = in_array(array_search($day, $dayIndex), $openDays);
                                @endphp
                                <div class="d-flex justify-content-between align-items-center">
                                    <p>{{ $day }}</p>
                                    @if ($isOpen)
                                        @foreach ($businessModel->hours as $hour)
                                            @if ($dayIndex[$hour->day_of_week] == $day)
                                                <p>
                                                    {{ $businessModel->convertTo12HourFormat($hour->opening_time) . ' a ' . $businessModel->convertTo12HourFormat($hour->closing_time) }}
                                                </p>
                                            @endif
                                        @endforeach
                                    @else
